...
>
    ...
    @include('layout.side' , ['slag' => 10 , 'subSlag' => 104])
    <div class="page-wrapper   @if(Config::get('app.locale') == 'ar') right @else  left  @endif ">
        @include('layout.cramp' , ['page_Title' => __('main.nationality_title') , 'menue' => __('main.side_basic') ])
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <table id="table" class="table table-striped table-bordered" style="width:100%">
                                ...
                                <tbody>
                                @foreach ($nationalties as $nationalty)
                                    <tr>
                                        <td class="text-center">{{ $loop->index + 1 }}</td>
                                        <td class="text-center">{{ $nationalty->id }}</td>
                                        <td class="text-center">{{ $nationalty->name_ar }}</td>
                                        <td class="text-center">{{ $nationalty->name_en }}</td>
                                        <td class="text-center">
                                             <button type="button" class="btn btn-success editBtn" value="{{$nationalty -> id}}"><i
                                                        class="fas fa-edit"></i></button>
                                           <button type="button" class="btn btn-danger deleteBtn" value="{{$nationalty -> id}}"><i
    ...
    @include('cpanel.Nationalty.create')
    @include('deleteModal')
</div>
...
<script type="text/javascript">
    ...
    function confirmDelete(){
        let url = "{{ route('destroyNationality', ':id') }}";
