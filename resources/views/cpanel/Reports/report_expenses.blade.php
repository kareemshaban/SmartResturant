...
<div id="main-wrapper" data-layout="vertical" data-navbarbg="skin5" data-sidebartype="full"
    ...
    <div class="page-wrapper" @if(Config::get('app.locale') == 'ar') style="margin-right: 250px; margin-left:0px;" @endif>
        @include('Layouts.subheader', [
            'pageTitle' => Config::get('app.locale') == 'ar' ? 'تقرير سندات صرف الصندوق  ': 'Report Expense Receipt' ])
            ...
            <div class="row justify-content-center">
                ...
                <div class="row justify-content-center">
                    <div class="col-md-8 col-xl-8 form data-entry">
                        ...
                        <form class="center" method="POST" action="{{ route('report_expenses') }}" enctype="multipart/form-data">
                            <div class="card-header px-0 mt-2 bg-transparent clearfix">
                                <h4 class="float-left pt-2">{{ __('main.report_expense_recipt') }}</h4>
                            ...
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <div class="col-6">
                                        <label>{{ __('main.expenses_type') }}</label>
                                        <input type="checkbox" name="is_expenses_type" id="is_expenses_type"/>
                                        <select name="expenses_type" id="expenses_type" class="form-select">
                                            <option selected value="0"> choose...</option>
                                            @foreach($expenses as $expense)
                                                <option selected value="{{$expense -> id}}"> {{ Config::get('app.locale') == 'ar' ?
                                                $expense -> name_ar : $expense -> name_en}}</option>
                                            @endforeach
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#is_expenses_type').prop('checked' , false);
        $('#expenses_type').attr('disabled' , true);
        $('#expenses_type').val('') ;
        ...
        $('#is_expenses_type').change(function (){
            $('#expenses_type').attr('disabled' , !this.checked);
        });
