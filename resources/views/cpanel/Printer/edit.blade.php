...
<div id="main-wrapper" data-layout="vertical" data-navbarbg="skin5" data-sidebartype="full"
    ...
    <div class="container-fluid">
        <form class="center" method="POST" action="{{ route('updatePrinter' , $printer -> id) }}" enctype="multipart/form-data" >

            <div class="row justify-content-center">
                @csrf <!-- {{ csrf_field() }} -->
                <div class="col-md-9 col-xl-7 data-entry">
                    <div class="card-header px-0 mt-2 bg-transparent clearfix">
                        <h4 class="float-left pt-2">{{__('main.edit_printer')}}</h4>
                    ...
                    <div class="card-body px-0">
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select mr-sm-2 @error('type') is-invalid @enderror"
                                    name="type" id="type">
                                <option value="1" @if($printer -> type == 1) selected @endif> {{__('main.printer_type1')}} </option>
                                <option value="2" @if($printer -> type == 2) selected @endif> {{__('main.printer_type2')}} </option>
                            </select>
                            @error('type')
                            <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
