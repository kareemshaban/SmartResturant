...
>
    ...
    @include('layout.side' , ['slag' => 10 , 'subSlag' => 1012])
    <div class="page-wrapper   @if(Config::get('app.locale') == 'ar') right @else  left  @endif ">
        @include('layout.cramp' , ['page_Title' => __('main.tables') , 'menue' => __('main.side_items') ])
                        ...
                        <div class="card-body">
                            ...
                            <table id="table" class="table table-striped table-bordered" style="width:100%">
                                ...
                                <tr>
                                    ...
                                    <th class="text-center">{{ __('main.available') }}</th>
                                ...
                                @foreach ($tables as $table)
                                        ...
                                        <td class="text-center">{{ $table->name_ar }}</td>
                                        <td class="text-center">{{ $table->name_en }}</td>
                                        <td class="text-center">{{  Config::get('app.locale') == 'ar' ?
                                              $table-> hall -> name_ar : $table-> hall -> name_en}}</td>
                                        <td class="text-center">{{$table -> available == 1 ? __('main.available1') : __('main.available2')}}</td>
                                    <td>
                                       <button type="button" class="btn btn-success editBtn" value="{{$table -> id}}"><i
                                                        class="fas fa-edit"></i></button>
                                            <button type="button" class="btn btn-danger deleteBtn" value="{{$table -> id}}"><i
                ...
                if(response){
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            $(".modal-body #hall_id").val(response.hall_id);
                        },
                        complete: function() {
                            $('#loader').hide();
                        },
