...
<body>
    ...
    <div id="main-wrapper" data-layout="vertical" data-navbarbg="skin5" data-sidebartype="full"
        ...
        @include('Layouts.subheader', [
                'pageTitle' => Config::get('app.locale') == 'ar' ? ' الطاولات' : 'Tables',
        ])
        <div class="container-fluid">
            <form class="center" method="POST" action="{{ route('updateTable', $table->id) }}"
                ...
                <div class="row justify-content-center">
                    ...
                    <div class="col-md-9 col-xl-7">
                        <div class="card-header px-0 mt-2 bg-transparent clearfix">
                            <h4 class="float-left pt-2">{{ __('main.edit_table') }}</h4>
                            <div class="float-right card-header-actions mr-1">
                                <button class="btn btn-primary" type="submit">
                                    <span class="ml-1">{{ __('main.edit_btn') }}</span>
                                </button>
                            </div>
                        </div>
                        <div class="card-body px-0">
                            ...
                            <div class="form-group">
                                ...
                                <select class="custom-select mr-sm-2 @error('hall_id') is-invalid @enderror"
                                    ...
                                    @foreach ($halls as $item)
                                        <option value="{{$item -> id}}"  @if($item -> id == $table ->hall_id) selected @endif>
                            ...
                            <div class="form-group">
                                <label>{{ __('main.available') }}</label>
                                <select class="custom-select mr-sm-2 @error('available') is-invalid @enderror"
                                        name="available" id="available">


                                        <option value="1"  @if( $table ->available == 1) selected @endif>
                                            {{ __('main.available1') }}</option>
                                    <option value="0"  @if( $table ->available == 0) selected @endif>
                                        {{ __('main.available2') }}</option>
    ...
    <script src="../../cpanel/plugins/bower_components/chartist-plugin-tooltips/dist/chartist-plugin-tooltip.min.js">
    </script>
